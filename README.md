# 🏥 Hospital Finder AI

> A smart web app that helps patients find the right hospital based on their health problem!

![Python](https://img.shields.io/badge/Python-3.x-blue?style=for-the-badge&logo=python)
![Flask](https://img.shields.io/badge/Flask-Web%20App-green?style=for-the-badge&logo=flask)
![Status](https://img.shields.io/badge/Status-Live-brightgreen?style=for-the-badge)

---

## 🌟 What This App Does

- ➕ **Add hospitals** with name, location, cost and insurance
- 🔍 **Search hospitals** by health problem
- 🏥 **View all hospitals** in the database
- 💰 Shows **treatment cost range**
- 🏦 Shows **insurance companies** accepted

---

## 🚀 How To Run This Project

### Step 1 — Clone the repository
```bash
git clone https://github.com/sanjaysandy533-dev/hospital-finder-ai.git
cd hospital-finder-ai
```

### Step 2 — Install Flask
```bash
pip install flask
```

### Step 3 — Run the app
```bash
python app.py
```

### Step 4 — Open browser
---

## 📸 Features

| Feature | Status |
|---|---|
| Add Hospital | ✅ Done |
| Search by problem | ✅ Done |
| View all hospitals | ✅ Done |
| Delete hospital | 🔄 Coming soon |
| Deploy online | 🔄 Coming soon |

---

## 👨‍💻 Developer

**Sanjay Sandy**
- 🌱 Beginner developer learning Python and AI
- 🎯 Building real projects step by step
- 💙 Goal: IT job with 1.5L/month salary

---

## 📅 Progress

| Day | What I Built |
|---|---|
| Day 1 | Python hospital search program |
| Day 2 | Full website with Flask + HTML |
| Day 3 | Coming soon... |

---

<p align="center">Made with 💙 | Beginner to Pro Journey 🚀</p>